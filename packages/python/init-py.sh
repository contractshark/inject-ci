...
pyenv() {
    ...
    if ! [ -e ~/.cache/pyenv/"$1"/contractshark ]; then
        pushd .
        cd || exit
        mkdir -p .cache/pyenv
        cd .cache/pyenv || exit
        ...
        PYPATH="$(pwd)"
        cd Python-* || exit
        ./configure --prefix="$PYPATH"/contractshark
        make
        make install
        popd || exit
    fi
    ...
    if ! [ -e ./bin/activate ]; then
        if [ -e "$HOME"/.cache/pyenv/"$1"/contractshark/bin/python3 ]; then
            virtualenv --python "$HOME"/.cache/pyenv/"$1"/contractshark/bin/python3 .
        elif [ -e "$HOME"/.cache/pyenv/"$1"/contractshark/bin/python2 ]; then
            virtualenv --python "$HOME"/.cache/pyenv/"$1"/contractshark/bin/python3 .
        else
            echo "error: no python binary in $HOME/.cache/pyenv/$1/contractshark/bin/"
            return
        fi
    fi
    source ./bin/activate
